...
def main():
    seasonfile = sys.argv[1]
    if not os.path.isfile(seasonfile):
        print('Need season CSV file', file=sys.stderr)
        exit(1)
    regular_season_df = pd.read_csv(seasonfile)
    append_stats_inplace(regular_season_df)
    regular_season_df.drop(columns=[''])
    # for col in regular_season_df.columns
    for season, 

def append_stats_inplace(df):
